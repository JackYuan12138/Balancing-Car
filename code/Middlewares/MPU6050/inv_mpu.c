...
#define MOTION_DRIVER_TARGET_MSP430		//采用MSP430的驱动
 ...
 */
int _mpu_run_self_test(long* gyro, long* accel)
{
#ifdef MPU6050
...
uint8_t MPU_Run_Self_Test(void)
{
    ...
    long gyro[3], accel[3];
    result = _mpu_run_self_test(gyro, accel);
    if (result == 0x3) {
...
uint8_t MPU_Write_Len(uint8_t addr, uint8_t reg, uint8_t len, uint8_t* buf)
{
    uint8_t i;
    MPU_IIC_Start();
    MPU_IIC_Send_Byte((addr << 1) | 0);//发送器件地址及写命令
...
uint8_t MPU_Read_Len(uint8_t addr, uint8_t reg, uint8_t len, uint8_t* buf)
{
    ...
    MPU_IIC_Wait_Ack();		//等待应答
    MPU_IIC_Start();
    MPU_IIC_Send_Byte((addr << 1) | 1);//发送器件地址及读命令
    MPU_IIC_Wait_Ack();		//等待应答
    while (len) {
...
{
    MPU_IIC_Start();
    MPU_IIC_Send_Byte((MPU_ADDR << 1) | 0);//发送器件地址及写命令
    if (MPU_IIC_Wait_Ack())	//等待应答
    {
        MPU_IIC_Stop();
        return 1;
...
uint8_t MPU_Read_Byte(uint8_t reg)
{
    ...
    MPU_IIC_Wait_Ack();		//等待应答
    MPU_IIC_Start();
    MPU_IIC_Send_Byte((MPU_ADDR << 1) | 1);//发送器件地址及读命令
    MPU_IIC_Wait_Ack();		//等待应答
